(function () {
  ...
  const SITE_HOSTNAME = 'durable-workflow.com';
  ...
  function writeConsent(value) {
    try {
      ...
      // still governs the current page without affecting rendering.
    }
  }
  ...
  function sendPageView() {
    ...
    const pagePath = normalizedPath();
    if (pagePath === runtime.lastPagePath) {
      return;
    }

    runtime.lastPagePath = pagePath;
  ...
  function schedulePageView() {
    if (runtime.navigationTimer) {
      window.clearTimeout(runtime.navigationTimer);
    }

    runtime.navigationTimer = window.setTimeout(function () {
  ...
  function observeClientNavigation() {
    if (runtime.navigationObserved || configuredPath) {
      return;
    }

    runtime.navigationObserved = true;
  ...
  function enableAnalytics() {
    if (runtime.analyticsEnabled || window.location.hostname !== SITE_HOSTNAME) {
      return;
    }
    ...
    if (!document.getElementById(LOADER_ID)) {
      ...
      loader.addEventListener('error', function () {
        // Analytics is optional. A blocked Google request must not affect the page.
      });
  ...
  function chooseConsent(value) {
    ...
    if (previous === 'granted' && runtime.analyticsEnabled) {
