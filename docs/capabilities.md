...
| Cancellation | Service-mode PHP, Python, and Rust clients expose immediate terminal cancellation with selected-run safety and typed outcomes. Embedded Laravel separately supports a cooperative cancellation request with bounded cleanup; Server and service-mode SDKs do not yet expose that request. | [Cancel and terminate](/docs/features/cancel-and-terminate/), [Rust SDK](/docs/polyglot/rust/) |
| Termination | Termination closes a run immediately with a distinct terminal outcome; it is exposed through Cloud and self-hosted Server service runtimes, CLI, PHP, Python, and Rust control surfaces. | [Cancel and terminate](/docs/features/cancel-and-terminate/), [Server API](/docs/polyglot/server-api-reference/) |
