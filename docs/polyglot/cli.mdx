...
For scripts, the task-queue JSON contract also includes
`stats.approximate_backlog_count`, `stats.approximate_backlog_age_seconds`,
`stats.tasks_added_last_minute`, and `stats.tasks_dispatched_last_minute`, so
automation can distinguish missing workers from admission throttling or a
queue whose durable input rate is outrunning dispatch:
