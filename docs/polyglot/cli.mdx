...
SDK, see [CLI and Python parity](/docs/2.0/polyglot/cli-python-parity).
...
dw upgrade --tag=0.1.8     # pin to a specific release tag
...
installs and points at the managing tool instead — for example
`composer update durable-workflow/cli` or
`brew upgrade durable-workflow/tap/dw`. See the
...
```bash
git clone https://github.com/durable-workflow/server.git durable-workflow-server
cd durable-workflow-server
...
dw workflow:describe quickstart-order-001
dw workflow:history quickstart-order-001
...
lease, poller, admission, and recent durable queue-flow state. Use it to
distinguish missing workers from admission throttling or a single queue that
is building backlog:
...
For fleet-level durable inflow versus dispatch rates across every queue, read
