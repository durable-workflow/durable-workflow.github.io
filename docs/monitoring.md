...
- final v2 writes signal and update lifecycle rows on the command path. Preview-era runs that predate `workflow_signal_records` or `workflow_updates` should be normalized on a compatible pre-clean-slate build before final v2 if Waterline detail, export, or operator actions need stable historical `signal_id` or `update_id` values
...
- if a preview-era accepted signal or update still only has command plus typed-history evidence, selected-run detail may show the command-based fallback identity. Final v2 does not include a lifecycle backfill command, so normalize those rows before upgrading when durable `workflow_signal` or `workflow_update` ids are required for historical operator flows.
