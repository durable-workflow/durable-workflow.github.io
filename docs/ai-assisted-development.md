...
the `-1.x.txt` URLs only when forward-compatibility against a future
major version matters; pin `-2.0.txt` only when the agent is specifically
building against the v2 alpha protocol.
